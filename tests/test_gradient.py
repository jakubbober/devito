...
class TestGradient(object):
    ...
    @pytest.mark.parametrize('dtype, opt', [(np.float32, 'advanced'), (np.float64, 'advanced')])
    def test_gradient_checkpointing(self, dtype, opt):
        ...
        gradient, _ = wave.jacobian_adjoint(residual, u0, vp=v0, checkpointing=True, grad=grad)

        grad = Function(name='grad', grid=wave.model.grid, dtype=np.float64)
        gradient2, _ = wave.jacobian_adjoint(residual, u0, vp=v0, checkpointing=False, grad=grad)
